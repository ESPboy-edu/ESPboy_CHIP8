...
uint8_t iskeypressed(uint8_t key){
 ...
 ret=0;
checkbuttons();
if (LEFT_BUTTON   && keys[LEFT_BUTTONn]   == key) ret++;
if (UP_BUTTON     && keys[UP_BUTTONn]     == key) ret++;
if (DOWN_BUTTON   && keys[DOWN_BUTTONn]   == key) ret++;
if (RIGHT_BUTTON  && keys[RIGHT_BUTTONn]  == key) ret++;
if (ACT_BUTTON    && keys[ACT_BUTTONn]    == key) ret++;
if (ESC_BUTTON    && keys[ESC_BUTTONn]    == key) ret++;
if (LFT_BUTTON    && keys[LFT_BUTTONn]    == key) ret++;
if (RGT_BUTTON    && keys[RGT_BUTTONn]    == key) ret++;
return (ret?1:0);
}
...
uint8_t waitanykey(){
  uint8_t ret;
  while (!checkbuttons()) delay(5);
  if (LEFT_BUTTON)  ret = keys[LEFT_BUTTONn];
  if (UP_BUTTON)    ret = keys[UP_BUTTONn];
  if (DOWN_BUTTON)  ret = keys[DOWN_BUTTONn];
  if (RIGHT_BUTTON) ret = keys[RIGHT_BUTTONn];
  if (ACT_BUTTON)   ret = keys[ACT_BUTTONn];
  if (ESC_BUTTON)   ret = keys[ESC_BUTTONn];
  if (RGT_BUTTON)   ret = keys[RGT_BUTTONn];
  if (LFT_BUTTON)   ret = keys[LFT_BUTTONn];
  return ret;
}
...
uint8_t readkey(){
  checkbuttons();
  if (LEFT_BUTTON) return LEFT_BUTTONn;
  if (UP_BUTTON) return UP_BUTTONn;
  if (DOWN_BUTTON) return DOWN_BUTTONn;
  if (RIGHT_BUTTON) return RIGHT_BUTTONn;
  if (ACT_BUTTON) return ACT_BUTTONn;
  if (ESC_BUTTON) return ESC_BUTTONn;
  if (LFT_BUTTON) return LFT_BUTTONn;
  if (RGT_BUTTON) return RGT_BUTTONn;
  if (!buttonspressed) return 255;
}
...
uint8_t drawsprite(uint8_t x, uint8_t y, uint8_t size)
{
  ...
  uint16_t addrdisplay; ret=0; preret=0;
    if (!size) size = 16;
    for(c=0; c<size; c++){
      data = mem[i+c];
      mask=128;
      ...
      for (d=0; d<8; d++){
        ...
        if ((x+d)<64 && (y+c)<32){
          if ((data & mask) && display[addrdisplay]) preret++;
...
{
  uint8_t data, mask, c, d, ret, preret;
  uint32_t addrdisplay; ret=0; preret=0;
    if (!size) size=16;
    for(c=0; c<size; c++){
      data = mem[i+c];
      mask=128;
      ...
      for (d=0; d<8; d++){
        ...
        if ((x+d)<64 && (y+c)<32){
          ...
          if(compatibility_emu & 8){
            if((x+d)>63) addrdisplay-=64;
            if ((y+c)>31) addrdisplay-=(32<<6);
            if((display[addrdisplay] && !(data & mask)) || (!display[addrdisplay] && (data & mask)))
               tft.fillRect((x+d)<<1, ((y+c)<<1)+16, 2, 2, colors[foreground_emu]);
            else
               tft.fillRect((x+d)<<1, ((y+c)<<1)+16, 2, 2, colors[background_emu]);    
      ...
      if (preret) ret++;
      if((y+c)>31 && !(compatibility_emu&64)) ret++;
    }
    if (compatibility_emu & 32) return (ret);
    else return (ret?1:0);
}
 ...
 uint8_t in1,x,y,zz;
  ...
  switch (in1){
    ...
    case 0x1: // jmp xyz
      pc=inst & 0x0FFF;
      break;
    case 0x3: // skeq:  skip next opcode if r(x)=zz
      if (reg[x] == zz) 
        pc+=2;
      break;
    case 0x4: //skne: skip next opcode if r(x)<>zz 
      if (reg[x] != zz) 
        pc+=2;
      break;
    case 0x5: //skeq: skip next opcode if r(x)=r(y)
      if (reg[x] == reg[y])
        pc+=2; 
      break;
    case 0x6: //mov
    ...
    case 0x7:  
            reg[x]=reg[x]+zz;
      break;
    case 0x9:  //skne: skip next opcode if r(x)<>r(y)
      if (reg[x] != reg[y]) 
        pc+=2;
      break;
    case 0xa: //mov
      ...
      break;
    case 0xb: //jmi xxx
      if (!compatibility_emu&16) pc=(inst & 0xFFF) + reg[0];
      else  pc=(inst & 0xFFF) + reg[(inst>>8)&0xF];
      break;
    case 0xc: //rand
      reg[x]= random(256) & zz;
      break;
    case 0xd: //draw sprite 
      ...
      break;
    case 0x0: //extended instruction 
      switch (zz){
        case 0xE0: 
          memset (&display[0], 0, 64*32);
          if (compatibility_emu&64) tft.fillRect(0, 16, 128, 64, colors[background_emu]);
          break;
        case 0xEE: // ret
          sp = --sp & 0xF;
          pc=stack[sp] & 0xFFF;
      ...
      break;
    case 0x8: //extended math instruction
      in2= inst & 0xF; 
      switch (in2){
        case 0x0: //mov r(x)=r(y)
          reg[x] = reg[y];
          break;
        case 0x1: //or
          reg[x] |= reg[y];
        ...
        case 0x4: //add
          ...
          if (x==0xf && compatibility_emu&4) reg[0xf] = r & 0xFF;
          break;
        case 0x5: //sub 
          r=reg[x] - reg[y];
          reg[x]=r & 0xff;
      ...
      break;
    case 0xe: //extended instruction
      switch (zz){
        case 0x9E: // skipifkey
          if (iskeypressed(reg[x])) pc+=2;
          break;
...
void do_emuation(){
  uint16_t c=0;
   timers.attach_ms((uint8_t)((float)1000/(float)timers_emu), chip8timers);
   ...
   while(1){
      if (c < opcodesperframe_emu) {  
          ...
          if (!(compatibility_emu&64)) updatedisplay();
          c=0;
       }
       if (LFT_BUTTON && RGT_BUTTON){
          chip8_reset();
          if (waitkeyunpressed() > 300) break;  
       } 
   }
    timers.detach();
}

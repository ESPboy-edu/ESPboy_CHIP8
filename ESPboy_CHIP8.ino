...
void updatedisplay(){    
  static uint16_t bufLine[256];
  static uint32_t i, j, addr, drawaddr, drawcolor, drawcolorback;

  addr=0;
  drawcolor = colors[foreground_emu];
  drawcolorback = colors[background_emu];
  drawcolor |= drawcolor << 16;
  drawcolorback |= drawcolorback << 16;
    ...
    case CHIP8:
      for (i = 0; i < screen_height; i++){
        for (j = 0; j < screen_width; j++){ 
          drawaddr = j<<1;
          if (display2[addr++])
            memcpy(&bufLine[drawaddr], &drawcolor, sizeof(uint32_t));
          else
            memcpy(&bufLine[drawaddr], &drawcolorback, sizeof(uint32_t));
        }
